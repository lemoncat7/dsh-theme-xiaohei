...
const sidebarTargetPath = new URL('../src/generated-sidebar-assets.ts', import.meta.url)
...
const sidebarAssets = [
  ['XIAOHEI_SIDEBAR_ATMOSPHERE', 'sidebar-spatial-atmosphere-v1.webp'],
  ['XIAOHEI_WORKSPACE_RIFT_COLOR', 'workspace-rift-color-v1.webp'],
]
const embeddedSidebarAssets = await Promise.all(sidebarAssets.map(async ([name, filename]) => {
  const assetPath = new URL(`../src/assets/sidebar/${filename}`, import.meta.url)
  const asset = await readFile(assetPath)
  return `export const ${name}: string = ${JSON.stringify(`data:image/webp;base64,${asset.toString('base64')}`)}`
}))
...
await writeFile(
  sidebarTargetPath,
  [
    '/** Generated by scripts/embed-assets.mjs for the sidebar atmosphere layer. */',
    ...embeddedSidebarAssets,
    '',
  ].join('\n'),
  'utf8',
)
