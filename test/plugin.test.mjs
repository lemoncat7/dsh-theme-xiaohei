...
test.skip('legacy Sylva Mixamo avatar contract', () => {
  ...
  assert.doesNotMatch(modeled, /xiaohei-avatar-soft-silhouette|THREE\.BackSide/)
  assert.doesNotMatch(modeled, /THREE\.HemisphereLight|THREE\.DirectionalLight/)
  assert.match(modeled, /intersectsSphere\(xiaoheiAvatarWorldSphere\)/)
...
test('conversation remains clear without a permanent reading veil', () => {
  ...
  assert.doesNotMatch(XIAOHEI_CONVERSATION_MESSAGES_CSS, /backdrop-filter:\s*blur/)
...
test('sidebar glass uses the quiet Realm material without stacking over the host', () => {
  ...
  assert.match(XIAOHEI_CHROME_TOKENS_CSS, /--xiaohei-sidebar-glass-fill:\s*rgb\(247 245 242 \/ 30%\)/)
  assert.match(XIAOHEI_CHROME_TOKENS_CSS, /--xiaohei-sidebar-glass-fill:\s*rgb\(18 22 25 \/ 25%\)/)
  assert.match(XIAOHEI_CHROME_TOKENS_CSS, /--xiaohei-sidebar-glass-saturation:\s*102%/)
  assert.match(XIAOHEI_CHROME_TOKENS_CSS, /--xiaohei-sidebar-glass-blur:\s*12px/)
