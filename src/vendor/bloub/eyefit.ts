...
import { EXPRESSIONS, type BotExpression } from './expressions.js'
import { eyePoses } from './face.js'
import { radiusAtAngle, toPoints, type Point } from './shape.js'
import { SHAPES } from './skins.js'
import { STATES, type Pose, type StateDef, type StateId } from './states.js'
...
// The theme uses the authored body, not the customiser. Avoid building its
// fitting catalogue at startup; preserve the solver for a future custom shape.
let DECALAGES: ReturnType<typeof batir> | undefined
const CATALOG_PROFILES = new Set(SHAPES.map(shape => shape.radii))
...
export function decalageDesYeux(
  ...
  if (!radii || !CATALOG_PROFILES.has(radii)) return NUL
  const par = (DECALAGES ??= batir()).get(radii)
  if (!par) return NUL
