// Adapted from bloub, Copyright (c) 2026 Jérémy Perret (MIT).
// See THIRD_PARTY_NOTICES.md for source revision and local changes.
...
export interface RenderedEye {
  ...
  matrix: string
  alpha: number
  /** Original local bounds, exposed for the theme's hollow ellipse renderer. */
  width: number
  height: number
  /** Uncompressed tangent and lid allow a stroked eye to close without vanishing. */
  openMatrix: string
  lid: number
}
...
function blendPose(a: Pose, b: Pose, t: number): Pose {
  ...
  return {
    ...
    // Theme adaptation: grow/shrink badge and transparent notch together,
    // including interrupted morphs, instead of flashing them at the midpoint.
    notif: a.notif && b.notif ? {
      x: lerp(a.notif.x, b.notif.x, t), y: lerp(a.notif.y, b.notif.y, t),
      r: lerp(a.notif.r, b.notif.r, t), notch: lerp(a.notif.notch, b.notif.notch, t),
    } : b.notif ? { ...b.notif, r: b.notif.r * t, notch: b.notif.notch * t }
      : a.notif ? { ...a.notif, r: a.notif.r * out, notch: a.notif.notch * out } : null,
    ...
    if (pose.eyeAlpha > 0.01) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        eyes.push({
          width: cfg.w * R,
          height: cfg.h * R,
          openMatrix: `matrix(${r2(ax)},${r2(ay)},${r2(cx2)},${r2(cy2)},${r2(e.x * fit + (offX + decalage.x) * R)},${r2(e.y * fit + (offY + decalage.y) * R)})`,
          lid: Math.min(lid, cfg.open),
